...
# Normalize the market index values to be between 0 and 1
market_index_normalized = (market_index - market_index.min()) / (market_index.max() - market_index.min())
...
# Calculate the 75th and 25th percentile thresholds for the normalized market index
high_threshold = market_index_normalized.quantile(0.75)
low_threshold = market_index_normalized.quantile(0.25)

# Plot a boxplot of the normalized market index values
plt.figure(figsize=(10, 5))
plt.boxplot(market_index_normalized)
...
plt.title('Normalized Market Index Boxplot with 75th and 25th Percentile Thresholds')
plt.ylabel('Normalized Market Index Value')
plt.legend()
plt.show()

# Define a function to determine if the market was high or low
def get_market_status(date, market_index_normalized, high_threshold, low_threshold):
    market_value = market_index_normalized.loc[date]
...
for peak in peaks:
    ...
    market_status = get_market_status(date, market_index_normalized, high_threshold, low_threshold)
