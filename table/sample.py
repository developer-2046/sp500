...
# Define a function to plot the correlation matrix and save it
def plot_correlation_matrix(correlation_matrix, sector_labels, sector_to_index, title, filename):
...
for start in range(0, len(log_returns) - window_size + 1, window_size):
    ...
    title = f"Correlation Matrix up to {last_date.strftime('%Y-%m-%d')}"
    filename = os.path.join(output_dir, f"{last_date.strftime('%Y-%m-%d')}.png")
    plot_correlation_matrix(correlation_matrix, sector_labels, sector_to_index, title, filename)

print(f"All correlation matrices have been saved in the '{output_dir}' directory.")
